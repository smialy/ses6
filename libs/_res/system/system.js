...
(function () {
  ...
  (function (global) {
    ...
    systemJSPrototype.getRegister = function () {
      ...
      return [[], function (_export) {
        return { execute() { _export('default', globalExport); } };
      }];
    };
  ...
  if (acquiringImportMaps) {
    Array.prototype.forEach.call(document.querySelectorAll('script[type="systemjs-importmap"][src]'), (script) => {
      script._j = fetch(script.src).then(resp => resp.json());
    });
  }
  ...
  systemJSPrototype.resolve = function (id, parentUrl) {
    ...
    if (acquiringImportMaps) {
      ...
      Array.prototype.forEach.call(document.querySelectorAll('script[type="systemjs-importmap"]'), (script) => {
        importMapPromise = importMapPromise.then(map => (script._j || script.src && fetch(script.src).then(resp => resp.json()) || Promise.resolve(JSON.parse(script.innerHTML)))
          .then(json => mergeImportMap(map, parseImportMap(json, script.src || baseUrl))));
      });
    }

    return importMapPromise
      .then(importMap => resolveImportMap(id, parentUrl, importMap));
  };
